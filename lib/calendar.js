 ...
 */
import { getAppCalendarDate } from './timezone';
...
export function getBirthdayDate(year = getAppCalendarDate().getFullYear()) {
  return new Date(year, getBirthdayMonthIndex(), getBirthdayDay());
}

/** Primer día del evento (día 31): 31 días antes del cumpleaños. */
export function getEventStartDate(year = getAppCalendarDate().getFullYear()) {
...
export function getCalendarDateForDayNumber(dayNumber, year = getAppCalendarDate().getFullYear()) {
