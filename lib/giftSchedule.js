...
export function applyGiftScheduleToDay(day, giftDayNumbers = getScheduledGiftDayNumbers()) {
  ...
  return {
    ...day,
    hasGift: true,
    // La categoría (nombre) la elige ella en el juego
    giftNumber: day.giftNumber ?? null,
  };
}

export function applyGiftScheduleToDays(days) {
  // Solo como respaldo si la BD no trae ningún hasGift (p.ej. seed vacío).
  // Si ya hay regalos en Supabase, no recalcular en cliente (evita spoilers/desync).
