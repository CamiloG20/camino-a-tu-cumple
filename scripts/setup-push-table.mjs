/**
 * Crea tabla push_subscriptions en Supabase.
 * Uso: npm run setup:push
 */
...
async function main() {
  ...
  const sql = readFileSync(
    resolve(root, 'supabase', 'migrations', '003_push_subscriptions.sql'),
    'utf8'
  );
  ...
  for (const host of hosts) {
    ...
    try {
      ...
      await client.end();
      console.log(`\n✅ Tabla push_subscriptions lista (${host})\n`);
      return;
    } catch (err) {
      lastError = err;
      try {
